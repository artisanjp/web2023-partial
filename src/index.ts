...
((window, document) => {
    ...
    const videos = document.querySelectorAll<HTMLVideoElement>(`video[${ TARGET_VIDEO_ATTRIBUTE }]`);

    // Only allow playback when all 3 videos are ready.
    let canPlay = false;
    
    // Playback is synced clock based to make it dead simple.
    let syncTimerStartedAt: number;
    ...
    const getIfAllVideosReady = () => {
        ...
        for (let i = 0; i < videos.length; i++) {
            ...
            if (video.readyState !== video.HAVE_ENOUGH_DATA) isReady = false;
        }
        return isReady;
    }
    ...
    const play = (video: HTMLVideoElement) => {
        if (!canPlay) return;
        if (!syncTimerStartedAt) throw new Error('syncTimerStartedAt is undefined, something\'s wrong with the first playback process');
    ...
    const checkIfInViewportAndControlPlayback = (video: HTMLVideoElement) => {
        ...
        if (isTopInViewport || isBottomInViewport) return play(video);
        pauseAndReset(video);
    }
    ...
    for (let i = 0; i < videos.length; i++) {
        ...
        const callback: IntersectionObserverCallback = (entries, observer) => {
            entries.forEach(entry => {
                if (!canPlay) return;
                if (entry.isIntersecting) return play( entry.target as HTMLVideoElement );
                pauseAndReset( entry.target as HTMLVideoElement );
            });
        };
...
((window, document) => {
    ...
    for (let i = 0; i < videos.length; i++) {
        ...
        const callback: IntersectionObserverCallback = (entries, observer) => {
            entries.forEach(entry => {                
                if (entry.isIntersecting) return play( entry.target as HTMLVideoElement );
                pause( entry.target as HTMLVideoElement );
            });
        };
        const observerOpts: IntersectionObserverInit = { 
            root: null, // defaults to viewport 
